...
void makeTensor(float *Tensor, TensorShape aShape)
{
	uint64_t offset;
...
__global__ void convLayer_gpu ( float * input, TensorShape iShape, float * filter, TensorShape fShape, 
	float * bias, float * output, TensorShape oShape, ConvLayerArgs args, uint32_t batchSize) 
{
...
float* Convolution(float* act, TensorShape actShape, TensorShape filterShape, ConvLayerArgs args)
{
	...
	uint32_t smFilH  = fShape.height;
	uint32_t smFilW  = fShape.width;
